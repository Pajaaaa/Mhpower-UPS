...
| _(working)_ | 1. 7. | **odolnost WiFi po výpadku brány (mAP)** — reakce na incident na zdroji Divišova (mAP vyplnut/zaplnut → ESP se nepřipojilo, nenaskočil ani hotspot, nutný fyzický restart). (1) `bindSnmpUdp`/`startMdns` **ven z WiFi event callbacku** (`GOT_IP` jen nastaví flag, dodělá se z `maintainWifi`) — lwIP/mDNS v event kontextu hrozilo deadlockem; (2) `startApFallback` **ověřuje návratovku `WiFi.softAP()`** a při selhání nenastaví `apFallbackActive` → hotspot se zkusí znovu (dřív se natvrdo „odškrtl“ a už nikdy nenaskočil); (3) throttle opakovaných pokusů o AP; (4) **zrychlené časování**: záchranný hotspot po **1 min** (dřív 5), tvrdý reboot jako poslední pojistka po **10 min** (dřív 1 h); (5) do event logu se při startu píše **důvod resetu** (`Start (brownout/panic/…)`) pro diagnostiku recidivy + **v1.18** |
