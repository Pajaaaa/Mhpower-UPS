...
| _(working)_ | 25. 6. | **oprava SNMP: `SNMP_MAX_INDEX` 48 → 49** — idx 49 (`runtimeProjectedSec`, proaktivní odhad výdrže) byl od v1.12 napsaný v `getSnmpValue`, ale cap na 48 ho přes SNMP nikdy nevystavil (GET .49 → bez odpovědi). Teď agent na .49 odpovídá; power monitor podle něj umí ukázat „odhad výdrže“ i na síti. README OID tabulka idx 49 už byla správně + **v1.17** |
| _(working)_ | 1. 7. | **odolnost WiFi po výpadku brány (mAP)** — reakce na incident na zdroji Divišova (mAP vyplnut/zaplnut → ESP se nepřipojilo, nenaskočil ani hotspot, nutný fyzický restart; **potvrzeno z živých dat**: `resetReason:task-watchdog` + power monitor viděl výpadek 09:02→09:31 CEST). (1) `bindSnmpUdp`/`startMdns` **ven z WiFi event callbacku** (`GOT_IP` jen nastaví flag, dodělá se z `maintainWifi`) — lwIP/mDNS v event kontextu hrozilo deadlockem, který wedgeuje driver a loop task pak visí >15 s → task-watchdog; (2) `startApFallback` **ověřuje návratovku `WiFi.softAP()`** a při selhání nenastaví `apFallbackActive` → hotspot se zkusí znovu (dřív se natvrdo „odškrtl“ a už nikdy nenaskočil); (3) throttle opakovaných pokusů o AP; (4) **zrychlené časování**: záchranný hotspot po **1 min** (dřív 5), tvrdý reboot jako poslední pojistka po **10 min** (dřív 1 h); (5) do event logu se při startu píše **důvod resetu** (`Start (task-watchdog/…)`) pro diagnostiku recidivy + **v1.18** |
| _(working)_ | 1. 7. | **nastavitelný vysílací výkon Wi-Fi ve webu** — dřív natvrdo `WIFI_POWER_5dBm`. Nová položka `settings.wifiTxQdbm` (¼ dBm, NVS klíč `wifiTx`, výchozí 20 = 5 dBm), tabulka povolených hodnot `WIFI_TX_OPTS` (2–19,5 dBm) + `isValidTxQdbm`/`currentTxPower`; `<select>` ve Wi-Fi sekci nastavení; **aplikuje se ihned bez restartu** (`WiFi.setTxPower` v POST handleru). Nízký výkon = menší proudové špičky/brownout u blízkého AP, vyšší = dosah. Kontrola přes SNMP idx 43 (TX dBm×10) / idx 17 (RSSI) + **v1.19** |
| _(working)_ | 2. 7. | **revizní balík oprav a optimalizací** (kompletní audit firmwaru): (1) **WDT se krmí i během boot čekání na WiFi** — čekání trvá až 15 s = celý WDT timeout; bez krmení by start bez dostupné WiFi (typicky po blackoutu, když AP naskakuje déle než ESP) skončil task-watchdog panikou dřív, než naskočí záchranný hotspot → boot smyčka; (2) **~16× rychlejší neúspěšný dekód**: `extractBits` (průchod 18k vzorků) vytažen z order/phase smyček plného hledání (`tryDecodeFromBits`) — volá se 44× místo 704×, k tomu early-skip při <96 bitech a krmení WDT v průběhu; méně sekání webu, znatelně nižší CPU při šumu; (3) **Peukert na síti respektuje 24V/48V banku** — fallback napětí `12.2 V` se škáluje `batterySystemVoltage/12` (jinak u vyšších řad proud nadsazen 2–4× → pesimistický `runtimeProjectedSec`); (4) **debounce přehřátí** (`confirmFlag`, 3 rámce) — stejná ochrana, jakou od v1.8 má alarm/přetížení; (5) dashboard už neukáže „full / **-1.0 V**" (JS test `>0`); (6) sanitizace `batDate` (uvozovka by rozbila JSON `/api/status`); (7) **uptime z 64bit `esp_timer_get_time()`** — web/SNMP idx 39 nepřeteče po 49,7 dnech `millis()`; (8) změna modelu s jiným napětím banky resetuje naučenou tabulku Wh/dílek + **v1.20** |
| _(working)_ | 14. 7. | **hloubková revize výpočtu výdrže + v1.21**: (1) **debounce dílků baterie** (3 rámce, `confirmBatteryBars`) — glitch jednoho rámce dřív zapsal falešný přechod do učení Wh/dílek (EMA váha 0,3) a resetoval počítadlo dílku; nečitelný vzor drží poslední známou hodnotu; (2) **nabíjecí animace už nerozhazuje SoC** — ven (SNMP idx 15, odhady) jde minimum z okna historie vzorů ≈ reálný stav, ne náhodná fáze animace (dřív skákala „autonomie" i % baterie při každém pollu monitoru); (3) **nečitelné dílky ≠ plná baterie** — `batteryBars >= 5` chytal i 255 (neznámé) a projekce hlásila výdrž z plné; teď `== 5` a při neznámém stavu `-1`; (4) **Peukert se nepočítá dvakrát** — učení kondice normalizuje `usedWh` deratingem při průměrné zátěži session (dřív: čím větší zátěž učicího výboje, tím pesimističtější všechny odhady); (5) **naučené Wh/dílek nesou i zátěž učení** (`learnedBarDrainW`, NVS `whBarW`) a predikce je koriguje poměrem Peukert deratingů na aktuální zátěž; prior dílků se nově deratuje taky; (6) **`avgDrainW` s časovou konstantou ~45 s** (dřív α=0,1/rámec ≈ τ 2 s — predikce skákala s každým přepnutím dílku zátěže); (7) **energetický model počítá se středem pásma dílku zátěže** (`modelLoadWatts`, dílek N = (N−0,5)·20 %; přetížení = 100 %) — displej/SNMP idx 12 dál ukazují horní hranici; (8) **SNMP idx 50 = verze firmwaru** (string, `FW_VERSION` jediné místo s verzí — web, `/api/status`, SNMP, dev-skeny); (9) **nastavení: login v samostatné sekci + ověřovací pole hesla** — heslo správce se změní jen při shodě obou polí (server- i client-side), překlep už neodřízne od webu; (10) explicitní prototypy `getSnmpValue`/`sendSnmpResponse`/`webAuthLevel` — kompilace i na novém arduino-cli (generované prototypy před definicemi typů) |
| _(working)_ | 3. 9. | **hlučný kus Čeperka 1 + v1.22**: kontrola po 49 dnech ukázala, že digit/icon-scan na hlučném kuse (~9 % rámců filtrováno) přetekly (uint16 = 65535, 12 slotů plných → nová hodnota už neměla kam) a že filtrem napětí prochází i posunutý rámec (`mode 0x10` + výstup 235 V místo 0x22 / 239 V). (1) **sanity kontrola rámce** `frameLooksCorrupt`: mem[6] musí mít bit 0x02 a mem[8] bity 0x63 (invarianty všech známých stavů; přidané neznámé bity — hledané V↑/V↓ — projdou; při zhaslém displeji se nevolá) — zahozené rámce v `rejected` (`/api/status`) a SNMP **idx 51**; (2) **debounce přepětí/podpětí** (`confirmFlag`, 3 rámce) — glitch „98 V" už na 1 rámec nezapne `underVoltage`/`avrState=boost`; (3) icon-scan: **uint32 čítače, 16 slotů, `maxrun`** (nejdelší souvislá řada rámců s hodnotou → odliší reálnou ikonu od šumu jednoho rámce), hi/lo záchyt chce 2 rámce po sobě mimo pásmo; digit-scan 12 slotů + uint32; oba `?reset=1`; (4) poznatek: všech 8 „neznámých" číslic Čeperky = známá číslice ± 1 bit (DP 0x80, segment e 0x02), žádný nový glyf; (5) **v1.23: DP bit 0x80 se v `digitFromPattern` maskuje** (žádná číslice ho nepotřebuje; 8 = 0x7F) — rámce s náhodným DP u 0/2/3/7/9 už nepadají do filtru jako „neznámá číslice" (na Čeperce ~80 k rámců za 49 dní) |
